...
def nifty_every_5():
    ...
    if (float(df.iloc[[-1]]['close']) < float(df.iloc[[-1]]['open']) and
        ...
        print(' Nifty Future Sell Recommanded Price is  :', float(min(df.iloc[-1:-8:-1]['low'])))
        ...
        print(' Nifty Future stoploss is :', df.iloc[(df.iloc[-1:-8:-1]['low'].idxmin()) - 1]['high'])
    ...
    # Continous last Seven candels in CLOSE greater than OPEN (GREEN)
        ...
        print(' Nifty Future Buy Recommanded Price is  :', float(max(df.iloc[-1:-8:-1]['high'])))
        # points = float(df.iloc[[2]]['high'] - df.iloc[[2]]['low'])
        ...
        print(' Nifty Future stoploss is :', df.iloc[(df.iloc[-1:-8:-1]['high'].idxmin()) - 1]['low'])
        f_list.append(datetime.date.today())
        f_list.append('NIFTY')
        f_list.append('BUY')
    ...
    book = openpyxl.load_workbook('nifty_5min_results.xlsx')
    sheet = book.active
